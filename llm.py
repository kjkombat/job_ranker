...
async def llm_response(job_description):
...
async def llm_generate_score(job_resume, scoring_criteria):
    async with Mistral(api_key=os.environ["MISTRAL_API_KEY"]) as client:
        scoring_criteria_cleaned = [
            re.sub(r"[^a-zA-Z0-9]", "_", s) for s in scoring_criteria
        ]
        scoring_class = create_class_from_strings(
            scoring_criteria_cleaned, "ScoreResume"
        )
        system_prompt = f"""Given the parsed job resume. Return the following
                      ...
                      """
        candidate_name = await client.chat.parse_async(
            model=model,
            messages=[
                {
                    "role": "system",
...
def create_class_from_strings(strings: List[str], class_name: str = "DynamicModel"):
    ...
    for string in strings:
        field_definitions[string] = (str, "5")
